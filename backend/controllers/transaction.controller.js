...
const createTransaction = asyncHandler(async (req, res) => {
  ...
  if (loanExists.status === "approved" || loanExists.status === "active") {
    res.status(400);
    throw new Error(
      "This loan has already been approved or is already active."
    );
  }
  ...
  if (type === "disbursement") {
    if (amount !== loanExists.amount) {
      res.status(400);
      throw new Error(
        `Disbursement amount must be exactly ${loanExists.amount}`
      );
    }

   // Set loan to active and initialize repaymentBalance
   loanExists.status = 'active';
   loanExists.repaymentBalance = loanExists.totalRepayable; // 👈 set it properly here
   await loanExists.save();
  }

  // Update user's repayableBalance
  await User.findByIdAndUpdate(user, {
    $inc: { repayableBalance: loanExists.totalRepayable },
  });
  ...
  // ✅ Fetch updated user to get current repayableBalance
  const updatedUser = await User.findById(user);

  res.status(201).json({
    message: "Transaction created successfully",
    transaction: newTransaction,
    repayableBalance: updatedUser.repayableBalance
  });
...
const updateTransaction = asyncHandler(async (req, res) => {
  ...
  if (!isValidOperation) {
    res.status(400);
    throw new Error(
      "Invalid updates. Only amount, type, status, and method are allowed."
    );
  }
...
const getTransactionsByLoan = asyncHandler(async (req, res) => {
  ...
  const transactions = await Transaction.find({ loan: loanId }).populate(
    "user",
    "name email"
  );
